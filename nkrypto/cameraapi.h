...
//              否则有可能造成内存溢出。
// 特别注意：CameraEnumerateDevice返回的相机信息列表，会根据acFriendlyName排序的，
//          例如可以将两个相机分别改为“Camera1”和“Camera2”的名字后，
//          CameraEnumerateDevice返回的列表，名字为“Camera1”的相机会排前面，名为“Camera2”的相机排后面，
//          这个方法是比较简单有效的，代码上就可以按顺序初始化了，不用进行特殊绑定了。
// 返回值   : 成功时，返回CAMERA_STATUS_SUCCESS (0);
//            否则返回非0值的错误码,请参考CameraStatus.h
//            中错误码的定义。
/******************************************************/
